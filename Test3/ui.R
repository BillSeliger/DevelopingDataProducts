...
h4('This app requires the user to input two values, demand for regular priced seats and demand 
...
The App will return the optimized decision variables - number of seats to be sold at each price level
...
h4('- We cannot sell more than 166 seats, the max capacity of the airplane'),
...
h4('In addition Regular priced seats are priced at US$617 and Discount seats at US$238'),
h4('This App is based on a lesson presented in the edX/MITx MOOC:'),
h3(a('MITx course The Analytics Edge', href="https://www.edx.org/course/analytics-edge-mitx-15-071x-0")),
h4('The lesson for this problem is solved in Excel in the video linked below'),
...
(LPs) with many thousands of variables and constraints and even Mixed Integer Programs (MIPs).  There are 
also several commercial solvers available - Gurobi and C-Plex being the most well known - that can be used for 
problems that are insolvable using the lpSolveAPI solver although it should be mentioned that many MIPs have proven 
intractable to even these commercial solvers.' ))),           
             tabPanel("LP Model",
                      mainPanel(
                        h3('The LP formulation submitted to the solver'),
